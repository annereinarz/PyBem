...
def K1(f,space_proj,time_proj):
    return lambda x,t: integrate(lambda y,s: f(y,s)*nfundamentalSol(space_proj.normal(y),x-y,t-s), space_proj, time_proj, n = 120, t=time_proj.inverse(t)) 

def K0(xsol, base,space_proj,time_proj):
  def K0func(x,t):
    Itotal = 0
    for alpha in range(base.nx):
        for m in range(base.nt):
             ...
             b1 = base.baseT[m].support[0](0);     b2 = base.baseT[m].support[0](1)
             def help(y,s):
                       return time_proj.derivative(s)*space_proj.derivative(y)*xsol[m*base.nx+alpha]*fundamentalSol(x-space_proj(y),t-time_proj(s))

             if t <= b2 and t> b1:
                       Itotal += integrate(help, base.baseX[alpha].support[0], base.baseT[m].support[0], n = 60, t=base.baseT[m].support[0].inverse(t),nsing=20)
             else:
                       Itotal += integrate(help, base.baseX[alpha].support[0], base.baseT[m].support[0], n = 60)
    return Itotal
  return K0func
